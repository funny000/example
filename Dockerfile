...
RUN /opt/conda/bin/mamba install -y -c conda-forge \
 ...
 && /opt/conda/bin/mamba install -y -c dglteam dgl-cuda11.7=0.9.1 \
...
RUN /opt/conda/bin/mamba install -c pytorch -y magma-cuda117 \
 && /opt/conda/bin/mamba clean -afy \
 ...
 && pip install --no-cache-dir -U \
    ...
    ${PT_TORCHDATA_URL} \
    torchnet \
    # fastai and accelerate must be pip installed with torch to avoid conda overwriting torch versions
    # fastai conda has a dependency chain of fastai -> spacy -> pathy -> dataclasses
    # conda path installs dataclasses regardless of py version: https://github.com/conda-forge/pathy-feedstock/blob/1e6da43da5c8e11e09c063c79b09236bbacb7780/recipe/meta.yaml#l29
...
RUN pip uninstall -y horovod \
 ...
 && HOROVOD_GPU_ALLREDUCE=NCCL HOROVOD_CUDA_HOME=/usr/local/cuda-11.7 HOROVOD_WITH_PYTORCH=1 pip install --no-cache-dir -U horovod==${HOROVOD_VERSION} \
 && ldconfig
...
RUN SMDATAPARALLEL_PT=1 pip install --no-cache-dir -U ${SMD_DATA_PARALLEL_URL}
